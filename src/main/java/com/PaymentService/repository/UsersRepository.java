...
@Query(value = "SELECT DISTINCT *from users where fullName=:fullName and userName=:userName And status=1", nativeQuery = true)
List<UsersEntity> findDistinctByFullNameAndUserNameNative(@Param("fullName") String fullName, @Param("userName") String userName);
   ...
   @Query(value = "SELECT *from users where userName=:userName LIKE 'N%' and status =:status",nativeQuery=true)
   List<UsersEntity> findByUserNameLikeAndStatus(@Param("userName") String pattern, @Param("status") Integer status);
